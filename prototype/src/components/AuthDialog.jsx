...
export function AuthDialog({ open, onClose }) {
  ...
  async function run(action, operation) {
    ...
    try {
      await operation();
      logEvent("auth.action_completed", { action, authMode });
    } catch (requestError) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-title" aria-describedby="auth-description" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <small>{authMode === "mock" ? "Local development mode uses explicit test identity providers." : "Authentication is completed by Google or a one-time email link."}</small>
      </section>
    </div>
  );
}
